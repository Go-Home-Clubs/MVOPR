...
SNR_range = seq(from=5,to=10,by=1)
...
w1 = create_sparsew1(p,q,r=r,ratio = 0.95,size = 2)
...
for(Sigma in c(0.7)){
  ...
  for(SNR in SNR_range){
    ...
    RR = Rank_search(M2, M1,r_range = r_range)
...
m1 = factor(rep(c('Adaptive Lasso','Cooperative(rho=0.5)','Cooperative(rho=1)','IntegFactor','Factor','MVOPR'),each=600), 
            levels=c('Adaptive Lasso','Cooperative(rho=0.5)','Cooperative(rho=1)','IntegFactor','Factor','MVOPR'))
