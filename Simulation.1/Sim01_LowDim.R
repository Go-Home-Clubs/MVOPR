...
SNR_range = seq(from=15,to=20,by=1)
...
w1 = create_sparsew1(p,q,r=r,ratio = 0.5,size = 1)
...
for(Sigma in c(0.7)){
  ...
  for(SNR in SNR_range){
    ...
    RR = Rank_search(M2, M1,r_range = 1:20)
...
m1 = factor(rep(c('Cooperative(rho=0)','Cooperative(rho=0.5)','Cooperative(rho=1)','Factor','IntegFactor','Multi-view'),each=1400), 
